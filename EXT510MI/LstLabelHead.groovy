/**
 * README
 * This extension is being used to List records from EXTSMF table. 
 *
 * Name: EXT510MI.LstLabelHead
 * Description: Listing records to EXTSMF table
 * Date	      Changed By                      Description
 *20230817  SuriyaN@fortude.co      Listing records from  EXTSMF table
 *
 */

public class LstLabelHead extends ExtendM3Transaction {
  ...
  public LstLabelHead(MIAPI mi, DatabaseAPI database, ProgramAPI program, MICallerAPI miCaller) {
    this.mi = mi
  ...
  /**
   *List records from EXTSMF table
   * @params 
   * @return 
   */
  public listRecord() {
    DBAction query = database.table("EXTSMF").selection("EXDIVI", "EXRIDN", "EXFACI", "EXWHLO", "EXPYCU", "EXSPOC", "EXIVNO", "EXITNO", "EXSPID", "EXCSID", "EXGTIN", "EXBXNO", "EXQUAN", "EXBANO", "EXUDF1", "EXUDF2", "EXUDF3", "EXUDF4", "EXCONO", "EXDLIX", "EXRIDL").index("00").build()
    DBContainer container = query.getContainer()

    if(!iWHLO.trim().isEmpty()&&!iRIDN.trim().isEmpty()&&(mi.inData.get("DLIX") != null && !mi.inData.get("DLIX").trim().isEmpty()))
    {
    ...
    else if(!iWHLO.trim().isEmpty()&&!iRIDN.trim().isEmpty())
    {
      container.set("EXDIVI", iDIVI)
      container.set("EXWHLO", iWHLO)
      container.set("EXRIDN", iRIDN)
