/**
 * README
 * This extension is being used to List records from EXTNCT table. 
 *
 * Name: EXT856MI.LstASNHead
 * Description: Listing records to EXTNCT table
 * Date	      Changed By                      Description
 *20230829  SuriyaN@fortude.co      Listing records from  EXTNCT table
 *
 */

public class LstASNHead extends ExtendM3Transaction {
  ...
  public LstASNHead(MIAPI mi, DatabaseAPI database, ProgramAPI program, MICallerAPI miCaller) {
    this.mi = mi
  ...
  /**
   *List records from EXTNCT table
   * @params 
   * @return 
   */
  public listRecord() {
    ...
    if (!iDIVI.trim().isEmpty() && !iWHLO.trim().isEmpty() && !iPYCU.trim().isEmpty()) {
    ...
    } else if (!iDIVI.trim().isEmpty() && !iWHLO.trim().isEmpty()) {
      container.set("EXDIVI", iDIVI)
      container.set("EXWHLO", iWHLO)
      container.set("EXCONO", iCONO)
