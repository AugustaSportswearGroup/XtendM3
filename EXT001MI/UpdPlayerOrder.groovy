...
public class UpdPlayerOrder extends ExtendM3Transaction {
  ...
  Closure < ? > resultset_LstLineDetails = {
    DBContainer container ->
      ...
      def params = ["CONO": CONO.toString().trim(),"DIVI":DIVI.toString().trim(),"ORNR":ORNR.toString().trim(),
                    "PLID":PLID.toString().trim(),"ORNO":iORNO.toString().trim(),"PONR":PONR.toString().trim(),"ITNO":ITNO.toString().trim()]
      def callback = {
        Map < String,
                String > response ->
          if(response.errorMessage!=null)
