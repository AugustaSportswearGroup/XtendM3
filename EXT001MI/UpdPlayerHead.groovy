...
public class UpdPlayerHead extends ExtendM3Transaction {
  ...
  private String iDIVI, iPLID, iORNR, iORNO, iOREF, iPLNM, iPLNU, iTEAM, iLEAG, iGRBY, iBGBY, iCUNO, iADID, iUDF1, iUDF2, iUDF3, iUDF4, iUDF5, iUDF6,iORDT
  private int iCONO, CHNO
  ...
  public void main() {
    ...
    iORDT = (mi.inData.get("ORDT") == null || mi.inData.get("ORDT").trim().isEmpty()) ? "" : mi.inData.get("ORDT")
  ...
  public validateInput() {
    ...
    if(!iORDT.trim().isEmpty())
    {
      //Validate Date
      String sourceFormat = utility.call("DateUtil","getFullFormat",mi.getDateFormat())
      validInput = utility.call("DateUtil","isDateValid",iORDT.trim(),sourceFormat.toString())
      if (!validInput)
      {
      ...
      }else
      {
        ...
        {
          iORDT = utility.call("DateUtil","convertDate",sourceFormat,"yyyyMMdd",iORDT.trim()) //Maintain date in YMD8 format in the table
        }
      }
    }
  }
   ...
   */
  public updateRecord() {
    DBAction query = database.table("EXTSTH").selection("EXCHNO").index("00").build()
    DBContainer container = query.getContainer()
  ...
  Closure < ? > updateCallBack = {
    LockedResult lockedResult ->
      ...
      if (iCONO != 0) {
        ...
        lockedResult.set("EXCONO", 0)
      }
      if (!iORDT.trim().isEmpty()) {
        lockedResult.set("EXORDT", iORDT as Integer)
      } else {
        lockedResult.set("EXORDT", 0)
      }
