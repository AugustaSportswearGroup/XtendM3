...
public class LstLPData extends ExtendM3Transaction {
  ...
  public void main() {
    ...
    if (validInput) {
      ...
      container.set("URDLIX", iDLIX as Long)
  ...
  /**
   *Validate inputs
   * @params int CONO , long DLIX
   * @return boolean
   */
  private boolean validateInput(int iCONO, long iDLIX) {
  ...
  private String getTemporaryOrderNumber(String RIDN) {
    String ORNO = ""
    Map<String, String> params = ["CONO": iCONO.toString().trim(), "ORNO": RIDN.toString().trim()]
    Closure<?> callback = {
      Map < String,
      String > response ->
  ...
  Closure < ? > updateDeliveryInformation = {
    ...
    lockedResult.set("EXDLIX", iDLIX as Long)
    lockedResult.update()
  }
