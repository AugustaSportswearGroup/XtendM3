/**
 * README
 * This extension is being used to Add records to EXTCUS table. 
 *
 * Name: EXT610MI.AddCustomerData
 * Description: Adding records to EXTCUS table 
 * Date	      Changed By                      Description
 *20230519  SuriyaN@fortude.co     Adding records to EXTCUS table */

import java.time.LocalDateTime
import java.time.format.DateTimeFormatter

public class AddCustomerData extends ExtendM3Transaction {
  ...
  public AddCustomerData(MIAPI mi, DatabaseAPI database, ProgramAPI program, MICallerAPI miCaller, UtilityAPI utility) {
    this.mi = mi
  ...
  public void main() {
    ...
    iULZO = (mi.inData.get("ULZO") == null || mi.inData.get("ULZO").trim().isEmpty()) ? "0" : mi.inData.get("ULZO")
    iPRLV = (mi.inData.get("PRLV") == null || mi.inData.get("PRLV").trim().isEmpty()) ? "0" : mi.inData.get("PRLV")
    iBLCD = (mi.inData.get("BLCD") == null || mi.inData.get("BLCD").trim().isEmpty()) ? "0" : mi.inData.get("BLCD")
  ...
  /**
   *Insert records to EXTCUS table
   * @params 
   * @return 
   */
  public insertRecord() {
    DBAction action = database.table("EXTCUS").index("00").build()
    DBContainer query = action.getContainer()

    query.set("EXCUNO", iCUNO)
    query.set("EXCUNM", iCUNM)
    query.set("EXADR1", iADR1)
    query.set("EXADR2", iADR2)
    query.set("EXADR3", iADR3)
    query.set("EXTOWN", iTOWN)
    query.set("EXECAR", iECAR)
    query.set("EXPONO", iPONO)
    query.set("EXCSCD", iCSCD)
    query.set("EXPHNO", iPHNO)
    query.set("EXTFNO", iTFNO)
    query.set("EXEMAL", iEMAL)
    query.set("EXTEPY", iTEPY)
    query.set("EXTXID", iTXID)
    query.set("EXULZO", iULZO)
    query.set("EXPRLV", iPRLV)
    query.set("EXBLCD", iBLCD)
