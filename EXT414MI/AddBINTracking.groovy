...
public class AddBINTracking extends ExtendM3Transaction {
  ...
  public validateInput() {
    ...
    if (!iMODL.trim().isEmpty()) {
      ...
      params = ["MODL": iMODL.toString().trim(), "LNCD": "GB"]
      callback = {
        Map < String,
        String > response ->
        if (response.MODL == null) {
          mi.error("Invalid Delivery Method " + iMODL)
          validInput = false
          return false
        }
      }
      miCaller.call("CRS070MI", "GetDelyMethod", params, callback)
    }
    ...
    if (!iTEPA.trim().isEmpty()) {
      ...
      query.set("CTDIVI", "")
      query.set("CTSTCO", "TEPA")
      query.set("CTSTKY", iTEPA)
      query.set("CTLNCD", "GB")
      if (!action.read(query)) {
        mi.error("Invalid Packaging Terms: " + iTEPA)
        return false
      }
    }
