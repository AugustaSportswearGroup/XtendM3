/**
 * README
 * This extension is being used to list records from EXTOPT table.
 *
 * Name: EXT003MI.LstScheduleInfo
 * Description: List records from EXTOPT table
 * Date	         Changed By                      Description
 *2023-02-03      SuriyaN@fortude.co          	 List records from EXTOPT table
 *
 */


public class LstScheduleInfo extends ExtendM3Transaction {
    private final MIAPI mi
    private final DatabaseAPI database
    private final ProgramAPI program
    private final MICallerAPI miCaller

    private int iSCHN, iMFNO, iCONO,pageSize = 10000
    private String iMTNO
    
    public LstScheduleInfo(MIAPI mi, DatabaseAPI database, ProgramAPI program, MICallerAPI miCaller) {
        this.mi = mi
    ...
    /**
     *List records from EXTOPT table
     * @params 
     * @return 
     */
    public listRecords() {
        ...
        DBContainer container  = query.getContainer()
         ...
         if(iSCHN!=0 && iMFNO!=0 && !iMTNO.trim().isEmpty())
         {
            container.set("EXCONO", iCONO)
            container.set("EXSCHN", iSCHN as Integer)
            container.set("EXMFNO", iMFNO as Integer)
            container.set("EXMTNO", iMTNO)
            query.readAll(container, 4, pageSize, resultset)
         }
        else if(iSCHN!=0 && iMFNO!=0 )
        {
          container.set("EXCONO", iCONO)
          container.set("EXSCHN", iSCHN as Integer)
          container.set("EXMFNO", iMFNO as Integer)
